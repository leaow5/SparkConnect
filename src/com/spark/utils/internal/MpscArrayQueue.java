...
final class MpscArrayQueue<E> extends MpscArrayQueueConsumerField<E> {
	 ...
	 * Lock free offer using a single CAS. As class name suggests access is
	...
	public boolean offer(final E e) {
		...
		do {
			...
			if (consumerIndexCache <= wrapPoint) {
				...
				if (currHead <= wrapPoint) {
					...
					// update on stack copy, we might need this value again if
					// we lose the CAS.
		...
		/*
		 * NOTE: the new producer index value is made visible BEFORE the element
		 * in the array. If we relied on the index visibility to poll() we would
		 * need to handle the case where the element is not visible.
		 */
	 ...
	 * @return 1 if next element cannot be filled, -1 if CAS failed, 0 if
	 *         successful
	 */
	 ...
	 * <p>
	 * IMPLEMENTATION NOTES:<br>
	 * Lock free poll using ordered loads/stores. As class name suggests access
	...
	public E poll() {
		...
		if (null == e) {
			/*
			 * NOTE: Queue may not actually be empty in the case of a producer
			 * (P1) being interrupted after winning the CAS on offer but before
			 * storing the element in the queue. Other producers may go on to
			 * fill up the queue after this element.
			 */
	 ...
	 * <p>
	 * IMPLEMENTATION NOTES:<br>
	 * Lock free peek using ordered loads. As class name suggests access is
...
abstract class MpscArrayQueueTailField<E> extends MpscArrayQueueL1Pad<E> {
	...
	static {
		try {
			P_INDEX_OFFSET = PlatformDependent0.UNSAFE
...
abstract class MpscArrayQueueConsumerField<E> extends MpscArrayQueueL2Pad<E> {
	...
	static {
		try {
			C_INDEX_OFFSET = PlatformDependent0.UNSAFE
