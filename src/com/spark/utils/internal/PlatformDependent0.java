...
import static com.spark.utils.StringTransformUtil.checkNotNull;
...
public class PlatformDependent0 {
	...
	static final int HASH_CODE_ASCII_SEED = 0xc2b2ae35; // constant borrowed
														// from murmur3
	...
	static {
		...
		if (addressField != null) {
			try {
				...
				try {
					if (unsafe != null) {
						unsafe.getClass().getDeclaredMethod("copyMemory", Object.class, long.class, Object.class,
	...
	static int hashCodeAscii(byte[] bytes, int startPos, int length) {
		...
		final int remainingBytes = length & 7;
		if (length > 7) { // Fast path for small sized inputs. Benchmarking
							// shows this is beneficial.
		...
		case 7:
			return ((hash * 31 + Integer.rotateLeft(hashCodeAsciiSanitize(UNSAFE.getInt(bytes, baseOffset + 3)), 13))
					* 31 + hashCodeAsciiSanitize(UNSAFE.getShort(bytes, baseOffset + 1))) * 31
					+ hashCodeAsciiSanitize(UNSAFE.getByte(bytes, baseOffset));
		case 6:
		...
		case 3:
			return (hash * 31 + hashCodeAsciiSanitize(UNSAFE.getShort(bytes, baseOffset + 1))) * 31
					+ hashCodeAsciiSanitize(UNSAFE.getByte(bytes, baseOffset));
		case 2:
	...
	/**
	 * This method assumes that {@code bytes} is equivalent to a {@code byte[]}
	 * but just using {@code char[]} for storage. The MSB of each {@code char}
	 * from {@code bytes} is ignored.
	 * <p>
	...
	static int hashCodeAscii(char[] bytes) {
		...
		for (int i = bytes.length - 8; i >= remainingBytes; i -= 8) {
			hash = hashCodeAsciiComputeFromChar(
					...
					UNSAFE.getLong(bytes, CHAR_ARRAY_BASE_OFFSET + (i + 4) * CHAR_ARRAY_INDEX_SCALE), hash);
		}
		switch (remainingBytes) {
		case 7:
			return ((hash * 31 + Integer.rotateLeft(hashCodeAsciiSanitizeFromChar(
					UNSAFE.getLong(bytes, CHAR_ARRAY_BASE_OFFSET + 3 * CHAR_ARRAY_INDEX_SCALE)), 13)) * 31
					+ hashCodeAsciiSanitizeFromChar(
							UNSAFE.getInt(bytes, CHAR_ARRAY_BASE_OFFSET + CHAR_ARRAY_INDEX_SCALE)))
					* 31 + hashCodeAsciiSanitizeFromChar(UNSAFE.getShort(bytes, CHAR_ARRAY_BASE_OFFSET));
		case 6:
			return (hash * 31 + Integer.rotateLeft(hashCodeAsciiSanitizeFromChar(
					UNSAFE.getLong(bytes, CHAR_ARRAY_BASE_OFFSET + 2 * CHAR_ARRAY_INDEX_SCALE)), 13)) * 31
					+ hashCodeAsciiSanitizeFromChar(UNSAFE.getInt(bytes, CHAR_ARRAY_BASE_OFFSET));
		case 5:
			return (hash * 31 + Integer.rotateLeft(hashCodeAsciiSanitizeFromChar(
					UNSAFE.getLong(bytes, CHAR_ARRAY_BASE_OFFSET + CHAR_ARRAY_INDEX_SCALE)), 13)) * 31
					+ hashCodeAsciiSanitizeFromChar(UNSAFE.getShort(bytes, CHAR_ARRAY_BASE_OFFSET));
		case 4:
			return hash * 31 + hashCodeAsciiSanitizeFromChar(UNSAFE.getLong(bytes, CHAR_ARRAY_BASE_OFFSET));
		case 3:
			return (hash * 31 + hashCodeAsciiSanitizeFromChar(
					UNSAFE.getInt(bytes, CHAR_ARRAY_BASE_OFFSET + CHAR_ARRAY_INDEX_SCALE))) * 31
					+ hashCodeAsciiSanitizeFromChar(UNSAFE.getShort(bytes, CHAR_ARRAY_BASE_OFFSET));
		case 2:
			return hash * 31 + hashCodeAsciiSanitizeFromChar(UNSAFE.getInt(bytes, CHAR_ARRAY_BASE_OFFSET));
		case 1:
			return hash * 31 + hashCodeAsciiSanitizeFromChar(UNSAFE.getShort(bytes, CHAR_ARRAY_BASE_OFFSET));
		default:
			return hash;
		}
	}
	...
	/**
	 * Callers are expected to call {@link #hasCharArray(CharSequence)} before
	 * calling this method.
	 */
	...
	/**
	 * Callers are expected to call {@link #hasByteArray(CharSequence)} before
	 * calling this method.
	 */
	...
	private static int hashCodeAsciiSanitizeFromChar(long value) {
		return (int) (((value & 0x1f000000000000L) >>> 24) | ((value & 0x1f00000000L) >>> 16)
				| ((value & 0x1f0000) >>> 8) | (value & 0x1f));
	}
	...
	static <U, W> AtomicReferenceFieldUpdater<U, W> newAtomicReferenceFieldUpdater(Class<? super U> tclass,
			String fieldName) throws Exception {
		return new UnsafeAtomicReferenceFieldUpdater<U, W>(UNSAFE, tclass, fieldName);
	}

	static <T> AtomicIntegerFieldUpdater<T> newAtomicIntegerFieldUpdater(Class<? super T> tclass, String fieldName)
			throws Exception {
		return new UnsafeAtomicIntegerFieldUpdater<T>(UNSAFE, tclass, fieldName);
	}

	static <T> AtomicLongFieldUpdater<T> newAtomicLongFieldUpdater(Class<? super T> tclass, String fieldName)
			throws Exception {
