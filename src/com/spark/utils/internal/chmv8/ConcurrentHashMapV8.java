...
import com.spark.utils.internal.IntegerHolder;
...
public class ConcurrentHashMapV8<K, V> implements ConcurrentMap<K, V>, Serializable {
	 ...
	 * elements were non-Comparable and had tied hashes.) The red-black
	 ...
	 * principle require only release ordering, not need full volatile
	...
	private final void addCount(long x, int check) {
		...
		if ((as = counterCells) != null || !U.compareAndSwapLong(this, BASECOUNT, b = baseCount, s = b + x)) {
			...
			if ((hc = threadLocals.counterHashCode()) == null || as == null || (m = as.length - 1) < 0
	...
	final Node<K, V>[] helpTransfer(Node<K, V>[] tab, Node<K, V> f) {
		...
		if ((f instanceof ForwardingNode) && (nextTab = ((ForwardingNode<K, V>) f).nextTable) != null) {
			if (nextTab == nextTable && tab == table && transferIndex > transferOrigin && (sc = sizeCtl) < -1
		...
		final TreeNode<K, V> putTreeVal(int h, K k, V v) {
			...
			for (TreeNode<K, V> p = root;;) {
				...
				if (p == null) {
					...
					if (p.left == null)
						dir = 1;
					else if ((pr = p.right) == null || (q = pr.findTreeNode(h, k, kc)) == null)
						dir = -1;
					else
						return q;
				}
	...
	private final void fullAddCount(InternalThreadLocalMap threadLocals, long x, IntegerHolder hc,
